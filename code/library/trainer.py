...
def train(
    ...
    total_preds = torch.zeros(args.batch_size * len(train_loader))
    total_targets = torch.zeros(args.batch_size * len(train_loader))
    batch_cnt = 0
    losses = 0
    loss_cnt = 0
    ...
    for step, batch in enumerate(train_loader):
        ...
        total_preds[step * args.batch_size : batch_cnt] = preds.detach()
        total_targets[step * args.batch_size : batch_cnt] = targets.detach()
        del preds
        del targets
        losses += loss.item()
        loss_cnt += 1

    total_preds = total_preds[:batch_cnt].cpu().numpy()
    total_targets = total_targets[:batch_cnt].cpu().numpy()
...
def recommend(model: nn.Module, seen: pd.Series, args) -> pd.DataFrame:
    ...
    if args.model.name.lower() in ["mf", "lmf"]:
        ...
        full_tensor = torch.concat((user_tensor, item_tensor), dim=1)
        del user_repeat, user_tensor, item_tensor
